...
// 这是Zhang Yin帮我修改的正确版本，linked_list.cpp 有一些地方有错误。包括main函数里print那边没有print出最后一个空的结构体
...
int main()
{
    student *pointer = create(); //type 1,2,3
    _print(pointer);
    pointer = insert(pointer, 0); //insert at front
    _print(pointer);
    pointer = insert(pointer, 7); //insert at end
    _print(pointer);
    pointer = insert(pointer, 5); //insert in the middle
    _print(pointer);
    pointer = dele(pointer, 4);
    _print(pointer);
    return 0;
}
